...
const VodWorkflow = () => {
  ...
  if (!vodData || !vodData.url || !vodData.thumbnail_url || !vodData.duration || !vodData.title) {
    return (
    ...
    try {
      const response = await fetch('http://localhost:8000/clips', {
        ...
        body: JSON.stringify({
          ...
          vod_username: vodData.display_name || vodData.user_login || vodData.user_name || '',
        }),
      });
      if (!response.ok) {
    ...
    <div className="min-h-screen bg-background-dark text-white flex flex-col items-center py-12 px-4 mt-20">
      ...
      <div className="w-full max-w-md flex flex-col items-center mx-auto">
        ...
        <div className="flex items-center justify-center w-full gap-8 mb-4">
          <div className="flex items-center gap-1">
            <span className="text-zinc-300 text-sm">Speech language:</span>
          ...
          <div className="flex items-center gap-1">
            <span className="text-zinc-300 text-sm">Credit usage:</span>
            <span className="text-amber-400 font-bold text-sm">⚡ 274</span>
          </div>
        </div>
        {/* Thumbnail Preview */}
      ...
      <div className="w-full px-4 flex flex-col items-center mt-8">
        <div className="w-full max-w-2xl bg-ring-background rounded-lg shadow-lg p-6">
          <div className="text-sm py-2 text-muted-foreground">Include specific moments</div>
          <div className="mb-4">
            <input
              type="text"
          ...
          <div className="flex flex-col gap-2">
            <div className="flex items-center gap-2 mb-2">
              <span className="bg-green-700 text-green-200 text-xs rounded px-2 py-0.5">Credit saver</span>
              <span className="text-zinc-400 text-xs">Processing timeframe</span>
            </div>
            <div className="flex items-center gap-2">
              <span className="text-zinc-400 font-mono">{secondsToHHMMSS(range[0])}</span>
              <div className="flex-1 px-2">
                <Slider
                  ...
                  step={1}
                  trackStyle={[{ backgroundColor: '#a78bfa', height: 6 }]}
                  handleStyle={[
                    { borderColor: '#a78bfa', backgroundColor: '#fff', height: 20, width: 20, marginTop: -7 },
                    { borderColor: '#a78bfa', backgroundColor: '#fff', height: 20, width: 20, marginTop: -7 },
                  ]}
                  railStyle={{ backgroundColor: '#27272a', height: 6 }}
                />
              </div>
              <span className="text-zinc-400 font-mono">{secondsToHHMMSS(range[1])}</span>
            </div>
          </div>
